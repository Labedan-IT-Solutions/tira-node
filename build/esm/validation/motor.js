import { validateRequired, validateEnum, validatePositiveNumber, validateNumber, validateDateString, validateDateRange, validatePhoneNumber, validateEmail, validateHttpsUrl, validateTaxesCharged, } from "./validators.js";
import { TiraValidationError } from "../errors.js";
export function validateMotorCoverNotePayload(payload) {
    // --- Top-level fields ---
    ...
    if (payload.covernote_type !== "1") {
        validateRequired(payload.previous_covernote_reference_number, "previous_covernote_reference_number");
    }
    ...
    // Premiums
    validatePositiveNumber(payload.total_premium_excluding_tax, "total_premium_excluding_tax");
    validatePositiveNumber(payload.total_premium_including_tax, "total_premium_including_tax");
    if (payload.total_premium_including_tax < payload.total_premium_excluding_tax) {
        throw new TiraValidationError("total_premium_including_tax must be greater than or equal to total_premium_excluding_tax.", "total_premium_including_tax");
    }
    ...
    if (payload.covernote_type === "3") {
        if (!payload.endorsement_type) {
            throw new TiraValidationError("Endorsement type is required for endorsements.", "endorsement_type");
        }
    ...
    if (!Array.isArray(payload.risks_covered) ||
        ...
        throw new TiraValidationError("At least one risk must be provided.", "risks_covered");
    }
    for (let i = 0; i < payload.risks_covered.length; i++) {
        ...
        validateNumber(r.premium_before_discount, `${label}.premium_before_discount`);
        validateNumber(r.premium_after_discount, `${label}.premium_after_discount`);
        validateNumber(r.premium_excluding_tax_equivalent, `${label}.premium_excluding_tax_equivalent`);
        ...
        if (r.discounts_offered) {
            if (!Array.isArray(r.discounts_offered)) {
                throw new TiraValidationError("discounts_offered must be an array.", `${label}.discounts_offered`);
            }
    ...
    if (!Array.isArray(payload.subject_matters_covered) ||
        ...
        throw new TiraValidationError("At least one subject matter must be provided.", "subject_matters_covered");
    }
    for (let i = 0; i < payload.subject_matters_covered.length; i++) {
        ...
        validateRequired(s.subject_matter_reference, `${label}.subject_matter_reference`);
    ...
    if (payload.covernote_addons) {
        if (!Array.isArray(payload.covernote_addons)) {
            throw new TiraValidationError("covernote_addons must be an array.", "covernote_addons");
        }
        for (let i = 0; i < payload.covernote_addons.length; i++) {
            ...
            validateNumber(a.premium_excluding_tax_equivalent, `${label}.premium_excluding_tax_equivalent`);
    ...
    if (!Array.isArray(payload.policy_holders) ||
        ...
        throw new TiraValidationError("At least one policy holder must be provided.", "policy_holders");
    }
    for (let i = 0; i < payload.policy_holders.length; i++) {
        ...
        validateDateString(p.policyholder_birthdate, `${label}.policyholder_birthdate`);
        ...
        validateRequired(p.policyholder_id_number, `${label}.policyholder_id_number`);
    ...
    if (m.motor_type === "1") {
        validateRequired(m.registration_number, "motor_details.registration_number");
    }
    ...
    if (m.motor_category === "1") {
        validateNumber(m.number_of_axles, "motor_details.number_of_axles");
        validateNumber(m.axle_distance, "motor_details.axle_distance");
        validateNumber(m.sitting_capacity, "motor_details.sitting_capacity");
    }
    if (isNaN(m.year_of_manufacture) ||
        m.year_of_manufacture < 1900 ||
        m.year_of_manufacture > new Date().getFullYear() + 1) {
        throw new TiraValidationError("A valid year of manufacture is required.", "motor_details.year_of_manufacture");
    }
